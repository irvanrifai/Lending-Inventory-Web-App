...
<div class="container">
    ...
    <div class="tab-content">
                    ...
                    <div class="col-xl-4 col-md-6 mb-4 mt-3">
                        <div class="card border-danger shadow h-100 py-2">
                            <div class="card-body">
                                <div class="row no-gutters align-items-center">
                                    <div class="col mr-2">
                                        <div class="text-xs font-weight-bold text-danger text-uppercase mb-1">Total inventaris finic ums</div>
        ...
        <div class="tab-pane fade" id="ajuan">
            ...
            <div class="row">
                <div class="col">
                    <h3>Daftar ajuan peminjaman aset ke FOSTI UMS</h3>
        ...
        <!-- menampilkan semua inventaris fosti -->
                ...
                <div class="col">
                    ...
                    <div class="table-responsive">
                        <table id="tb_inv" class="table table-striped">
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                <?php foreach ($aset as $a_finic) : ?>
                                    <tr>
                                        <td scope="row"><?= $i++; ?></td>
                                        <th><img src="/img/<?= $a_finic['gambar']; ?>" class="gambar"></th>
                                        <td><?= $a_finic['id_inventaris']; ?></td>
                                        <td><?= $a_finic['nama_aset']; ?></td>
                                        <td><?= $a_finic['jumlah_kapasitas']; ?></td>
                                        <td><?= $a_finic['status']; ?></td>
                                        <td><?= $a_finic['kondisi']; ?></td>
                                        <td><?= ($a_finic['cb_nb'] == '1') ? 'Bisa' : 'Tidak'; ?></td>
                                        <td>
                                            <button type="button" class="bi bi-pencil-square btn btn-warning tooltip-test" title="Edit data" data-bs-toggle="modal" data-bs-target="#edit<?= $a_finic['nomor']; ?>">

                                            </button>
                                            <button type="button" class="bi bi-trash btn btn-danger tooltip-test" title="Hapus data" data-bs-toggle="modal" data-bs-target="#hapus<?= $a_finic['nomor']; ?>">
...
<div class="modal fade" id="tambah" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form novalidate action="/admin/adm_fosti/save" role="form" method="POST" enctype="multipart/form-data" id="form_tambah" class="needs-validation">
                <?= csrf_field(); ?>
                <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                <input type="hidden" name="kepemilikan" id="kepemilikan" value="FOSTI UMS">
...
<!-- modal untuk edit -->
<?php foreach ($aset as $a_fosti) : ?>
    <div class="modal fade" id="edit<?php echo $a_fosti['nomor']; ?>" tabindex="-1" role="dialog" aria-labelledby="editLabel">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="/admin/adm_fosti/update/<?= $a_fosti['nomor']; ?>" method="POST" role="form" enctype="multipart/form-data" class="needs-validation">
                    <?= csrf_field(); ?>
                    <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                    <input type="hidden" name="kepemilikan" id="kepemilikan" value="FOSTI UMS">
                    <input type="hidden" name="nomor" value="<?= $a_fosti['nomor']; ?>">
                    <input type="hidden" name="gambar_lama" value="<?= $a_fosti['gambar']; ?>">
                    <div class="modal-body">
                        <div class="mb-3">
                            <div class="mb-3">
                                <label class="mb-2"><b>Pilih Foto Aset</b></label>
                                <div class="col-sm-4">
                                    <img src="/img/<?= $a_fosti['gambar']; ?>" class="img-thumbnail img-preview">
                                </div>
                                <div class="mt-2 col-md-10">
                                    <input type="file" class="form-control <?= (\Config\Services::validation()->hasError('gambar')) ? 'is-invalid' : ''; ?>" id="gambar" name="gambar" value="<?= (old('gambar')) ? old('gambar') : $a_fosti['gambar']; ?>" onchange="preview_img()">
                                    <div class="invalid-feedback">
                                        <?= \Config\Services::validation()->getError('gambar'); ?>
                                    </div>
                                    <div id="emailHelp" class="form-text">Direkomendasikan untuk memasukkan foto.</div>
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                <label for="id_inventaris" class="form-label"><b>Nomor Inventaris</b></label>
                                <input type="text" class="form-control <?= (\Config\Services::validation()->hasError('id_inventaris')) ? 'is-invalid' : ''; ?>" id="id_inventaris" name="id_inventaris" value="<?= (old('id_inventaris')) ? old('id_inventaris') : $a_fosti['id_inventaris']; ?>">
                                <div class="invalid-feedback">
                                    <?= \Config\Services::validation()->getError('id_inventaris'); ?>
                                </div>
                            </div>
                            <div class="mb-3 col-md-6">
                                <label for="nama_aset" class="form-label"><b>Nama Aset</b></label>
                                <input type="text" class="form-control <?= (\Config\Services::validation()->hasError('nama_aset')) ? 'is-invalid' : ''; ?>" id="nama_aset" name="nama_aset" value="<?= (old('nama_aset')) ? old('nama_aset') : $a_fosti['nama_aset']; ?>">
                                <div class="invalid-feedback">
                                    <?= \Config\Services::validation()->getError('nama_aset'); ?>
                                </div>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="jumlah_kapasitas" class="form-label"><b>Jumlah</b></label>
                                <input type="number" min="1" class="form-control <?= (\Config\Services::validation()->hasError('jumlah_kapasitas')) ? 'is-invalid' : ''; ?>" id="jumlah_kapasitas" name="jumlah_kapasitas" value="<?= (old('jumlah_kapasitas')) ? old('jumlah_kapasitas') : $a_fosti['jumlah_kapasitas']; ?>">
                                <div class="invalid-feedback">
                                    <?= \Config\Services::validation()->getError('jumlah_kapasitas'); ?>
                                </div>
                            </div>
...
<!-- modal untuk hapus -->
<?php foreach ($aset as $a_fosti) : ?>
    <div class="modal fade" id="hapus<?= $a_fosti['nomor']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/admin/adm_fosti/delete/<?= $a_fosti['nomor']; ?>" method="post" role="form" enctype="multipart/form-data">
                        <?= csrf_field(); ?>
                        <input type="hidden" name="nomor" value="<?php echo $a_fosti['nomor']; ?>">
                        <input type="hidden" name="gambar_lama" value="<?= $a_fosti['gambar']; ?>">
                        <div class="modal-body">
                            <div class="mb-3">
                                <div class="mb-3">
                            ...
                            <div>
                                <label for="id_inventaris" class="form-label">Nomor Inventaris : <?= $a_fosti['id_inventaris']; ?></label>
                            </div>
                            <div class="mb-3">
                                <label for="nama_aset" class="form-label">Nama Aset : <?= $a_fosti['nama_aset']; ?></label>
                            </div>
    ...
    <div class="modal fade" id="remove<?= $l['id_peminjam']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/admin/adm_fosti/remove/<?= $l['id_peminjam']; ?>" method="post" role="form" enctype="multipart/form-data">
                        <?= csrf_field(); ?>
...
<?php foreach ($laporan_p as $l) : ?>
    <div class="modal fade" id="detail<?= $l['id_peminjam']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form action="/admin/adm_fosti/update_a/<?= $l['id_peminjam']; ?>" role="form" method="GET" enctype="multipart/form-data">
                <div class="modal-content">
                    <div class="modal-header">
...
<div class="modal fade" id="rentang_p" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
    <form method="POST" action="/admin/adm_fosti/index">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($aset as $a_fosti) : ?>
                                        <tr>
                                            <td scope="row"><?= $i++ ?></td>
                                            <th><img src="/img/<?= $a_fosti['gambar']; ?>" class="gambar"></th>
                                            <td><?= $a_fosti['id_inventaris']; ?></td>
                                            <td><?= $a_fosti['nama_aset']; ?></td>
                                            <td><?= $a_fosti['jumlah_kapasitas']; ?></td>
                                            <td><?= $a_fosti['status']; ?></td>
                                            <td><?= $a_fosti['kondisi']; ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
