...
class adm_bem extends BaseController
{
    protected $asetModel;
    protected $HomeModel;
    ...
    public function index()
    {
        ...
        $dari_a = $this->request->getVar('dr_filter_a');
        $sampai_a = $this->request->getVar('sp_filter_a');

        $kepemilikan = ['BEM FKI UMS'];
        ...
        $data = [
            ...
            'dari_a' => $dari_a,
            'sampai_a' => $sampai_a,
            'home' => $this->HomeModel->filter_laporan($dari_a, $sampai_a),
            ...
            'most_b_j' => $this->PeminjamanModel->sering_jumlah($kepemilikan),
            'total_admin' => $this->UserModel->hitung('users'),
            'total_agenda' => $this->HomeModel->hitung('agenda'),
            'laporan_p_j' => $this->asetModel->aset_pinjam()->distinct()->get()->getResultArray(),
        ];

        // dd($data['laporan_p_j']);
        return view('pages/admin/adm_bem', $data);
    }

    // tambah buat inventaris
    public function save()
    {
        ...
        if (!$this->validate([
            ...
            return redirect()->to('/pages/admin/adm_bem')->withInput();
        }
    ...
    public function tambah()
    {
        ...
        if (!$this->validate([
            'agenda' => [
                ...
                'errors' => [
                    ...
                    'max_length' => 'Nama agenda terlalu panjang (75 Char)'
                ]
            ],
            'penyelenggara' => [
                'rules' => 'required|max_length[75]',
                'errors' => [
                    'required' => 'Penyelenggara agenda tidak boleh kosong..',
                    'max_length' => 'Penyelenggara agenda melebihi ukuran maksimal (75 Char)'
                ]
            ],
            'waktu' => [
                'rules' => 'required',
            ...
            'jam' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Jam pelaksanaan tidak boleh kosong.'
                ]
            ],
            'tempat' => [
                'rules' => 'required|max_length[75]',
                'errors' => [
                    ...
                    'max_length' => 'Tempat pelaksanaan terlalu panjang (75 Char)'
                ]
            ]
        ])) {
            ...
            // $validation = \Config\Services::validation();
            // return redirect()->to('/pages/admin')->withInput()->with('validation', $validation);
            session()->setFlashdata('Fail_m', ' Data agenda baru gagal ditambahkan.');
            return redirect()->to('/pages/admin/adm_bem')->withInput();
        }
        ...
        session()->setFlashdata('Success_m', ' Data agenda baru berhasil ditambahkan.');
        return redirect()->to('/pages/admin/adm_bem');
    }
    ...
    {
        // cek ID inventaris
        // $inventaris_lama = $this->adm_bemModel->getadm_bem($this->request->getVar('nomor'));
        ...
        if (!$this->validate([
            ...
            return redirect()->to('/pages/admin/adm_bem/' . $this->request->getVar('nomor'))->withInput();
        }
    ...
    // edit buat agenda
    public function ubah($id_agenda)
    {
        $data = [
            'validation' => \Config\Services::validation(),
            'home' => $this->HomeModel->getHomeModel($id_agenda),
            'title' => 'Admin Ormawa | SPOKI UMS'
        ];
        return view('pages/admin/adm_bem', $data);
    }

    public function perbaharui($id_agenda)
    {
        //validasi read the manual book CI 4
        if (!$this->validate([
            'agenda' => [
            ...
            // $validation = \Config\Services::validation();
            // return redirect()->to('/pages/admin')->withInput()->with('validation', $validation);
            session()->setFlashdata('Fail_m', ' Data agenda gagal diedit.');
            return redirect()->to('/pages/admin/adm_bem/' . $this->request->getVar('id_agenda'))->withInput();
        }
        ...
        session()->setFlashdata('Success_m', ' Data agenda berhasil diedit.');
        return redirect()->to('/pages/admin/adm_bem');
    }

    // hapus buat inventaris
    ...
    {
        $this->HomeModel->delete($id_agenda);
        session()->setFlashdata('Success_m', ' Data agenda berhasil dihapus.');
        return redirect()->to('/pages/admin/adm_bem');
